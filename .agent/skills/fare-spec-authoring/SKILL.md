...
4. **Vị trí đẩy** — khảo sát resource `knowledge-tree` + `list_plan_items`; đề xuất vị trí + **CHỜ User chốt** (`§2`). KHÔNG mặc định. Lưu ý: `glossary` thường nằm ở `scope="project"` (1 doc / project); `brd` thường ở `scope="project"`; `srs` / `user_story` thường gắn `plan_item_id` cấp Function (rule §1).
5. **Tạo** — `create_document(doc_type=..., purpose=..., content=..., status="draft")`:
   ...
   - Richtext (`brd` / `srs` / `prd`) → để `content` TRỐNG cho FARE inject template hệ thống; sau đó `edit_document` điền từng block.
