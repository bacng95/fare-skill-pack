...
## Xác định `bug_origin` TRƯỚC (INTRINSIC vs EXTRINSIC)

Mỗi bug phải xác định nguồn gốc — quyết định nó có chặn task khác hay không (xem `fare-mcp-integration`):

| `bug_origin` | Khi nào | `linked_task_id` | Tác động |
|---|---|---|---|
| **INTRINSIC** (nội sinh) | Bug phát sinh khi đang làm/verify 1 task cha — **TC fail trong `/fare-verify`**, hoặc lỗi lộ ra khi test feature của task đó | **BẮT BUỘC** = task cha (task `type=TEST` hoặc feature task gắn TC) | Chặn task cha chuyển DONE đến khi bug đóng |
| **EXTRINSIC** (ngoại lai) | Bug độc lập — production incident, regression không gắn task đang làm, user report ngẫu nhiên | Không cần | Không chặn task nào |

**Default theo nguồn phát hiện:**
- **TC fail (từ `fare-test-execution`)** → mặc định **INTRINSIC**, `linked_task_id` = task TEST gốc (hoặc feature task chứa TC). TC fail nghĩa là feature chưa đạt AC → bug phải chặn task cha DONE.
- **Bug dạo thử / ngoài TC (qua `/fare-qa`)** → hỏi User: thuộc 1 task đang làm (INTRINSIC) hay độc lập (EXTRINSIC)?
- Không chắc → hỏi User. KHÔNG đoán bừa origin.
...
4. **Báo cáo Markdown + HỎI User** (rule §5):
   ...
   ```
5. **CHỜ User trả lời.**
   - User OK → `create_tasks(tasks=[{type:"BUG", bug_origin:"INTRINSIC", linked_task_id:<task cha>, ...}])` (batch — kể cả 1 BUG, §4). Sau khi có id → `update_test_case(testCaseId=TC fail, verify={..., linked_task_id=<bug_id>})` để TC fail có truy ngược đến BUG.
   - INTRINSIC bug bắt buộc có `linked_task_id` — thiếu → lỗi validation.
