...
**Cú pháp:** `/fare-write-doc [mã project] [loại tài liệu?] [plan item id?]`
...
3. Agent chạy SOP: xác nhận loại tài liệu + plan item đích → khảo sát ngữ cảnh (skill `fare-context-discovery`) → viết đúng format / schema → đồng bộ FARE (doc mặc định `draft`).
