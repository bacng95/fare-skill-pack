...
4. **Verify sau đẩy (BẮT BUỘC):** `read_document` đọc lại từng doc — (a) **KHÔNG còn dấu hiệu RAW** (`<table>` nguồn / nhãn "Overview/Basic Flow" / bullet `*`·`+` literal) = chưa normalize → làm lại doc đó; (b) mỗi `**Bước N:**` là đoạn riêng (KHÔNG dính 1 dòng); (c) chip link hiện **1 cái** (không nhân đôi). Sai → `edit_document` sửa.
